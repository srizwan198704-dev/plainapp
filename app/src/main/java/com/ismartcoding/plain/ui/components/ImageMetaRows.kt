...
fun ImageMetaRows(path: String) {
    ...
    meta?.let { mt ->
        ...
        if (mt.flash > 0) {
            PListItem(title = stringResource(Res.string.flash), value = ImageHelper.getFlashText(mt.flash))
        }
        if (mt.fNumber > 0) {
            PListItem(title = stringResource(Res.string.f_number), value = "f/" + FormatHelper.formatDouble(mt.fNumber, 1))
        }
        PListItem(title = stringResource(Res.string.exposure_program), value = ImageHelper.getExposureProgramText(mt.exposureProgram))
        PListItem(title = stringResource(Res.string.metering_mode), value = ImageHelper.getMeteringModeText(mt.meteringMode))
        PListItem(title = stringResource(Res.string.white_balance), value = ImageHelper.getWhiteBalanceText(mt.whiteBalance))
